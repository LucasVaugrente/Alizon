    ...
    $tab_reponses_commentaire = verif_reponses_commentaires($_COOKIE['id_commentaire']);
        if(isset($_COOKIE['texte_reponse'])){
            if (sizeof($tab_reponses_commentaire) == 0){
                ...
                echo '<script> alert("Vous avez déjà répondu à ce commentaire !"); </script>'; 
            }
        } 
    }
    ...
    if(isset($_COOKIE['texte_commentaire'])){
        if (isset($_SESSION['id_client'])){
            if (sizeof(verif_commentaires($_SESSION['id_client'], $_GET['idProduit'])) == 0){
                ...
                echo '<script> alert("Vous avez déjà posté un commentaire !"); </script>'; 
            }
        }       
    }
        ...
        <main class="main_produit">
                    ...
                    <article class="infos">
                        <aside>
                            ...
                            <form class="ajoutPanierPageProduit" action="produit.php?idProduit=<?php echo $produit["id_produit"]; ?>&qteSelect=<?php echo $qteSelect; ?>&produitajouté" method="POST">
                                ...
                                <?php else:?>
                                    <button name="ajoutPanier" type="submit" class="ajout" disabled>Indisponible</button>
                                <?php endif; ?>
                            </form>
                        <?php endif; ?>
                        </aside>
                        ...
                        <form action="catalogue.php?id_produit=<?php echo $produit["id_produit"]; ?>" method="POST">
                            ...
                            <?php else:?>
                                <button name="ajoutPanier" type="submit" class="ajout-tel" disabled>Indisponible</button>
                            <?php endif; ?>
                        </form>
                        <?php endif; ?>
            ...
            <section class="commentaires">
                ...
                <article class='carte_commentaire'>
                    ...
                    <?php   
                        $listeReponses = reponses_avis($tabComplet[$i]['ID_Client']); 
                        $nbrReponses = sizeof($listeReponses); 
                    ?>
                    <?php for ($j=0; $j < $nbrReponses; $j++): ?>
                        <hr>
                        <h3> <?php echo "Réponse du Vendeur"; ?> </h3>
                        <h4> <?php echo $listeReponses[$j]['Commentaire']; ?></h4>
                    <?php endfor; ?>
                </article>            
                <?php endfor;?>
