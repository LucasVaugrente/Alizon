    ...
    <main class="main-catalogue">
        ...
        <section class="milieu_catalogue">
            <div class="liste-filtre">
                ...
                <div class="titre_filtres_resp">
                    ...
                    <img src="../img/fermer.png" alt="Logo Femrer Fenetre" title="Fermer les filtres" width='25px'
                        height='25px' class="fermer_filtres_cat">
                </div>
                <form name="filters" method="get">
                    <div class="categorie-catalogue">
                        ...
                        <details class="details0">
                            ...
                            <?php foreach ($vendeurs as $vendeur): ?>
                                <div class="sous-categorie categorie-vendeur" for=<?php echo ($vendeur['Nom_vendeur']); ?>>
                                    ...
                                    <label for=<?php echo ($nom_vendeur_propre); ?>
                                        class="sous-categorie-catalogue"><?php echo ($vendeur["Nom_vendeur"]); ?></label>
                                    <input name=<?php echo ($nom_vendeur_propre); ?> type="checkbox"
                                        id=<?php echo ($nom_vendeur_propre); ?>
                                        onchange="checkFiltre('<?php echo ($nom_vendeur_propre); ?>')">
                                </div>
                            <?php endforeach; ?>
                        ...
                        <?php for ($i = 0; $i < sizeof($listeCat); $i++): ?>
                            <details class="details<?php echo $i + 1; ?>">
                                <summary class="categorie">
                                    <h2><?php echo $listeCat[$i]["Nom_Categorie"]; ?></h2>
                                </summary>

                                <!-- Liste des sous-catégories -->
                                <?php for ($j = 0; $j < sizeof($liste_Sous_Cat); $j++) : ?>
                                    <?php if ($liste_Sous_Cat[$j]["Id_Categorie_Sup"] == $listeCat[$i]["ID_Categorie"]): ?>
                                        <div class="sous-categorie categorie-produit"
                                            for="<?php echo $liste_Sous_Cat[$i]["nom"] ?>">
                                            <label for="<?php echo str_replace(' ', "_", $liste_Sous_Cat[$j]["nom"]); ?>"
                                                class="sous-categorie-catalogue"><?php echo $liste_Sous_Cat[$j]["nom"] ?></label>
                                            <?php $liencat = str_replace(' ', "_", $liste_Sous_Cat[$j]["nom"]); ?>
                                            <input name="<?php echo str_replace(' ', "_", $liste_Sous_Cat[$j]["nom"]); ?>"
                                                class="checkbox" type="checkbox"
                                                id="<?php echo str_replace(' ', "_", $liste_Sous_Cat[$j]["nom"]); ?>"
                                                onchange='checkFiltre("<?php echo $liencat ?>")'>
                                        </div>
                                    <?php endif; ?>
                                <?php endfor; ?>
                            </details>
                    ...
                    <form class="range_container" method="get" name="FiltrePrix" id="FiltrePrix">
                        <div class="sliders_control">
                            <input id="fromSlider" type="range" value="<?php echo "$min" ?>" min="0" max="999"
                                onchange="checkFiltre(null)" />
                            <input id="toSlider" type="range" value="<?php echo "$max" ?>" min="0" max="999"
                                style="z-index: 0;" onchange="checkFiltre(null)" />
                        </div>
                        <div class="form_control">
                            <div class="form_control_container">
                                <div class="form_control_container__time">Min</div>
                                <input class="form_control_container__time__input" type="number" id="fromInput"
                                    value="<?php echo "$min" ?>" min="0" max="999" onchange="checkFiltre(null)" />
                            </div>
                            <div class="form_control_container">
                                <div class="form_control_container__time">Max</div>
                                <input class="form_control_container__time__input" type="number" id="toInput"
                                    value="<?php echo "$max" ?>" min="0" max="999" onchange="checkFiltre(null)" />
                            </div>
                        </div>
                    </form>

                    <form action="catalog.php" method="Get" class="form_suppr_filtres">
                ...
                <div class="boutons_resp_filtres">
                    ...
                    <form action="catalog.php" method="Get" class="form_suppr_filtres">
                        <button class="normal-button rouge supprimer-filtre resp_filtres_suppr">Supprimer tous les
                            filtres</button>
                    </form>
                </div>
            ...
            <div class="article_cat">
                <?php foreach ($catalogue as $id => $produit): ?>
                    <div id="<?php echo $id; ?>"
                        class="carte_produit_ajout <?php echo str_replace(' ', "_", $produit["nom_souscategorie"]); ?> <?php echo $produit["Nom_vendeur"]; ?>">

                        <a class="carte_produit_lien" href="product.php?idProduit=<?php echo $produit["id_produit"]; ?>">
                            <img src="<?php echo str_replace(' ', "_", "../img/catalogue/Produits/" . $produit['id_produit'] . "/" . $produit["images1"]); ?>"
                        ...
                        <div class="carte_produit_ajout--boutons__panier">
                            ...
                            <button class="infoVendeurProduit"> Vendu et expédié par <?php echo $vendeur['Nom_vendeur']; ?>
                            </button>
                            <form id="formAction" action="catalog.php?<?php if (isset($terme)) {
                                                                            echo 'id_produit=' . $produit["id_produit"] . '&terme=' . $terme;
                                                                        } else {
                                                                            echo 'id_produit=' . $produit["id_produit"];
                                                                        } ?>&produitajouté" method="POST">
                ...
                <div class="block-CatalogueVide">
                    ...
                    <div class="block-texte_panierVide">
                        <img src="../img/catalogueVide.png" alt="Catalogue vide" title="Catalogue vide" width="350"
                            height="300px">
        ...
        <div class="filtres_prix_responsives_block">
            <div id="sorting-options_resp">
                <div class="boutons_tri">
                    <button id="ascending-sort" class="boutonTri_resp" onclick="sortProducts('asc', event)">Prix
                        croissant</button>
                    <hr>
                    <button id="descending-sort" class="boutonTri_resp" onclick="sortProducts('desc', event)">Prix
                        décroissant</button>
                </div>
            </div>
        </div>
...
<script>
    ...
    let tabElem = [];
    let elem = document.getElementsByClassName("carte_produit_ajout");
    ...
    function checkFiltre(elem) {
        ...
        if (elem == null) {
            ...
            if (checkboxTab.length == 0) {
                ...
                let cartes = document.getElementsByClassName("carte_produit_ajout");
                ...
                for (let indexCat = 0; indexCat < cartes.length; indexCat++) {
                    ...
                    let prix = cartes[indexCat].childNodes[5].childNodes[3].childNodes[3].innerHTML.slice(0, -1);
                ...
                for (let j = 0; j < checkboxTab.length; j++) {
                    ...
                    if (boolVendeurCoché == true) {
                        ...
                        for (let indexSupprCat = 0; indexSupprCat < tabCat.length; indexSupprCat++) {
                            ...
                            tabClassProduit = [];
                            for (let indextabClassProduit = 0; indextabClassProduit < tabCat[indexSupprCat].classList
                                .length; indextabClassProduit++) {
                            ...
                            for (let nbVendeursChecked = 0; nbVendeursChecked < tab_vendeur_checked
                                .length; nbVendeursChecked++) {
                                if (tabClassProduit.includes(tab_vendeur_checked[nbVendeursChecked])) {
                                    boolProduitVendeurValidé = true;
                                }
                            }
                            for (let nbCategoriesChecked = 0; nbCategoriesChecked < tab_categories_checked
                                .length; nbCategoriesChecked++) {
                ...
                for (let j = 0; j < checkboxTab.length; j++) {
                    ...
                    for (let indexCat = 0; indexCat < tabCat.length; indexCat++) {
                        let prix = tabCat[indexCat].childNodes[5].childNodes[3].childNodes[3].innerHTML;
                        prix = prix.slice(0, -1);
                        let pointenmoins = prix.indexOf(".");
                        if (pointenmoins != null) {
                            if (pointenmoins == 3) {
            ...
            let sousCat = document.getElementsByClassName("sous-categorie");
            let boolC = false;
        ...
        let boolAff = false;
        let vide = document.getElementsByClassName("block-CatalogueVide")[0];
        let sousCat = document.getElementsByClassName("carte_produit_ajout");
    ...
    function sortProducts(order) {
        let products = document.getElementsByClassName("carte_produit_ajout");

        let sortedProducts = Array.prototype.slice.call(products).sort(function(a, b) {
            let priceA = parseFloat(a.childNodes[5].childNodes[3].childNodes[3].innerHTML);

            let priceB = parseFloat(b.childNodes[5].childNodes[3].childNodes[3].innerHTML);
        ...
        let parent = document.getElementsByClassName("article_cat")[0];
        console.log(parent);
