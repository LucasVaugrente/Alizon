...
<body>
    ...
    <main class="main-GestionCompte">
                        ...
                        <?php if ($liste_Client[$i]['active'] == 1): ?>
                            <?php if ($active == $i): ?>
                                <form action="GestionCompte.php?<?php echo 'id=' . $liste_Client[$i]['ID_Client'] ?>" method="POST">
                                    <button class="ButtonGestionCompte active" name="numList" value="<?php echo $i; ?>"
                                        type="submit">
        ...
        <div class="bloc_infos_client">
            ...
            <form action="GestionCompte.php" method="GET" class="search-box-cli">
                <input type="search" name="client_terme" class="search-txt-cli" placeholder="Rechercher un client..."
                    required>
            ...
            <div class="bloc_infos_persos_client_gestion">
                ...
                <h1>Coordonnees de
                    <?php echo strtoupper($infoClient['nom_client']) . " " . $infoClient['prenom_client']; ?>
                </h1>

                <div class="coordonnees">

                    <form method="POST"
                        action="GestionCompte.php?<?php echo 'id=' . $infoClient['ID_Client'] . '&numList=' . $active; ?>">
                        <div class="formulaire">

                            <div class="champ_infos_persos">
                                <label for="fname">Nom</label>
                                <input id="fname" type="text" name="nom"
                                    placeholder=<?php echo str_replace(" ", "-", $infoClient['nom_client']); ?>>
                            </div>

                            <div class="champ_infos_persos">
                                <label for="fname">Prénom</label>
                                <input id="fname" type="text" name="prenom"
                                    placeholder=<?php echo str_replace(" ", "-", $infoClient['prenom_client']); ?>>
                            </div>

                            <div class="champ_infos_persos">
                                <label for="fname">Email <i class="fa-solid fa-triangle-exclamation" style="color: red;"
                                ...
                                <input type="email" name="email" readonly="readonly"
                                    ...
                                    value=<?php echo str_replace(" ", "-", $infoClient['email']); ?>>

                            </div>

                            <div class="champ_infos_persos">
                                <label for="fname">Date de naissance</label>
                                <input id="fname" type="date" name="DateDeNaissance"
                                    value=<?php echo str_replace(" ", "-", $infoClient['date_de_naissance']); ?>>
                            </div>

                            <div class="champ_infos_persos">
                                <label for="fname">Adresse</label>
                                <input id="fname" type="text" name="adresse"
                                    placeholder=<?php echo str_replace(" ", "-", $infoClient['adresse_facturation']); ?>>
                            </div>
                    ...
                    <form
                        action="GestionCompte.php?<?php echo 'ID_Client=' . $infoClient['ID_Client'] . '&ID_Panier=' . $infoClient['ID_Panier']; ?>"
                        method="POST" class="btn-suppr" name="formSupp">
                        <input type="submit" class="supprimer" name="supp" id="supp" onClick="verifDeco()"
    ...
    <script>
        function verifDeco() {
            let text =
                "Voulez-vous vraiment désactiver le compte de <?php echo $infoClient['nom_client'] . " " . $infoClient['prenom_client']; ?>";
