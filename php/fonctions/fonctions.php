<?php 
    ...
    function details_commande($id) {
        ...
        $sth = $dbh->prepare("SELECT * from $schema._commande natural join $schema._contient_produit_c natural join $schema._produit where ID_Client = ? and etat_commande = ?");
        $sth->execute(array($id,"en cours"));
        ...
        return array('listeCommandeTotale'=> $commandesToute, 'listeCommande'=> $commandes, 'listeCommandeArch'=> $commandesArch, 'nbCommandeCours'=>sizeof($commandeCours), 'nbCommandeArch'=>sizeof($commande));
    }
    ...
    function inscription($email, $nom, $prenom, $ddn, $mdp, $cmdp, $rep,$adr_facturation, $complementAdresse_facturation, $ville_facturation, $codePostal_facturation, $adr_livraison = NULL, $complementAdresse_livraison= NULL, $ville_livraison= NULL, $codePostal_livraison= NULL)
    {
        ...
        if ($verif >= 1) {
            ...
            if($mdp === $cmdp){
                ...
                // AJOUT DES ADRESSES 
                if(empty($complementAdresse_facturation)){
                    $sth = $dbh -> prepare("INSERT INTO Alizon._Adresse (ID_Client,nom_de_rue,ville,code_postale) VALUES (?,?,?,?)");
                    $sth->execute(array($id_client,$adr_facturation,$ville_facturation,$codePostal_facturation));
                }else{
                    $sth = $dbh -> prepare("INSERT INTO Alizon._Adresse (ID_Client,nom_de_rue,complement,ville,code_postale) VALUES (?,?,?,?,?)");
                    $sth->execute(array($id_client,$adr_facturation,$complementAdresse_facturation,$ville_facturation,$codePostal_facturation));
                }

                if($adr_livraison != '' || $adr_livraison != NULL){
                    $sth = $dbh -> prepare("INSERT INTO Alizon._Adresse (ID_Client,nom_de_rue,complement,ville,code_postale) VALUES (?,?,?,?,?)");
                    $sth->execute(array($id_client,$adr_livraison,$complementAdresse_livraison,$ville_livraison,$codePostal_livraison));
                }
            }
        }
        return $res;
    ...
    function creer_commande($id_client, $etat_commande, $adresse_livraison)
    {
        global $schema;
        global $dbh;

        $sth = $dbh->prepare("INSERT INTO alizon._commande(ID_Client, etat_commande, adresse_livraison)VALUES(?, ?, ?)");
        $sth->execute(array($id_client, $etat_commande, $adresse_livraison));
    }
    ...
    function afficher_produit_vendeur($i)
    {
        global $schema;
        global $dbh;

        $sth = $dbh->prepare("select ID_Commande,_contient_produit_c.ID_Produit,Nom_produit,Quantite,Prix_produit_commande_TTC from _contient_produit_c inner join _produit on _contient_produit_c.ID_Produit = _produit.ID_Produit where ID_Commande=$i;");
        $sth->execute();
        $result = $sth->fetchAll();
    ...
    function afficher_commandes_vendeur()
    {
        ...
        $sth = $dbh->prepare("SELECT _client.ID_Client, nom_client, prenom_client, date_commande, etat_commande, _commande.adresse_livraison, prix_total from $schema._client INNER JOIN $schema._commande ON _client.ID_Client = _commande.ID_Client ;");
        $sth->execute();
        $result = $sth->fetchAll();
    ...
    function update_etat_commande($id, $etat) {
        global $schema;
        global $dbh;

        $sth = $dbh->prepare("UPDATE alizon._commande SET etat_commande = '$etat' WHERE ID_Commande= $id;");
        $sth->execute();
        $result = $sth->fetchAll();
