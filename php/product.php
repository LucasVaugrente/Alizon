...
<body>
                ...
                <article class="infos">
                    ...
                    <aside>
                        ...
                        <?php if (!isset($_SESSION["vendeur"]) == true): ?>
                            <form class="ajoutPanierPageProduit"
                                action="product.php?idProduit=<?php echo $produit["id_produit"]; ?>&qteSelect=<?php echo $qteSelect; ?>&produitajouté"
                                method="POST">
                                <?php if ($produit["quantite_disponnible"] >= 1): ?>
                                    <button name="ajoutPanier" type="submit" class="ajout">Ajouter au panier</button>
                                <?php else: ?>
                                    <button name="ajoutPanier" type="submit" class="ajout" style="background-color: #CB333B"
                    ...
                    <?php if (!isset($_SESSION["vendeur"]) == true): ?>
                        <div class="produit-quantité">
                            <?php if ($produit["quantite_disponnible"] >= 1): ?>
                                <h5>Quantité</h5>
                            <?php endif; ?>
                            <form action="product.php?idProduit=<?php echo $produit["id_produit"]; ?>" method="POST"
                                class="quantite_prod">
                                <?php if ($produit["quantite_disponnible"] >= 1): ?>
                    ...
                    <?php if (!(isset($_SESSION["id_admin"])) && !(isset($_SESSION["vendeur"]))): ?>
                        <form
                            action="product.php?idProduit=<?php echo $produit["id_produit"]; ?>&qteSelect=<?php echo $qteSelect; ?>&produitajouté"
                            method="POST" class="form_ajout_tel">
                            <?php if ($produit["quantite_disponnible"] >= 1): ?>
                                <button name="ajoutPanier" type="submit" class="ajout-tel">Ajouter au panier</button>
                            <?php else: ?>
                                <button name="ajoutPanier" type="submit" class="ajout-tel" style="background-color: #CB333B"
                                    disabled>Indisponible</button>
            ...
            ?>
                <article class='carte_commentaire'>
                    ...
                    <?php if (isset($_SESSION['id_admin'])) : ?>
                        ...
                        <form action="product.php?idProduit=<?php echo $produit["id_produit"]; ?>" method="POST"
                            id="supprimer<?php echo $i + 1; ?>" name="formSupprime">
                            <input type="hidden" id="commentaireId<?php echo $i; ?>" name="SupprCommentaireId"
                                value=<?php echo $tabComplet[$i]['ID_Commentaire']; ?> />
                            <button class="signaler" type="submit" value=<?php echo $i; ?>
                                name="Supprimer<?php echo $i; ?>">Supprimer</button>
                        </form>
                    <?php else : ?>
                        ...
                        <?php if (!(isset($_SESSION["vendeur"]))): ?>
                            <?php if (isset($_SESSION["id_client"])): ?>
                                <form
                                    action="product.php?idProduit=<?php echo $produit["id_produit"]; ?>#Ancre_ID_Commentaire<?php echo $i; ?>"
                                    method="post" id="signaler<?php echo $i + 1; ?>" name="formSignale">
                                    <input type="hidden" id="commentaireId<?php echo $i; ?>" name="CommentaireId"
                                        value=<?php echo $tabComplet[$i]['ID_Client']; ?> /><!-- ici -->
                                    <button class="signaler" type="submit" id="signalementButton<?php echo $i; ?>"
                                        <?php if (SignaleurComment($_SESSION['id_client'], $tabComplet[$i]['ID_Commentaire']) != null) {
                                            echo "disabled style='background-color : #eee; color : #CB333B'";
                                        } ?>>
                                        <?php if (SignaleurComment($_SESSION['id_client'], $tabComplet[$i]['ID_Commentaire']) == null) {
                                            echo "Signaler";
                                        } else {
                            ...
                            <?php else: ?>
                                <form action="login.php" method="post" id="signaler<?php echo $i + 1; ?>" name="formSignale">
                                    <input type="hidden" id="commentaireId<?php echo $i; ?>" name="CommentaireId"
                                        value=<?php echo $tabComplet[$i]['ID_Client']; ?> /><!-- ici -->
                                    <button class="signaler" type="submit" id="signalementButton<?php echo $i; ?>"
                                        <?php if (SignaleurComment($_SESSION['id_client'], $tabComplet[$i]['ID_Commentaire']) != null) {
                    ...
                    <?php if ((isset($_SESSION["vendeur"])) && (sizeof(verif_reponses_commentaires($tabComplet[$i]['ID_Commentaire'])) == 0)): ?>
                        <!-- Bouton ajout d'une réponse à un commentaire -->
                        <!-- id_commentaire -->
                        <button id="réponse<?php echo $i; ?>" class="réponse" type="submit" type="button"
                            value=<?php echo $tabComplet[$i]['ID_Commentaire']; ?>
                            name="Répondre<?php echo $i; ?>">Répondre</button>
                    <?php endif; ?>
                    <article class="bloc-reponse">
                        <label id="label-reponse<?php echo $i; ?>" class="reponse-comm"> Réponse </label>
                        <input id="texte-reponse<?php echo $i; ?>" class="reponse-comm-texte" type="text" name="reponse"
                            required>
                        <button id="btn-valider-reponse<?php echo $i; ?>" class="reponse-comm-btn-valider" type="submit">
                            Envoyer </button>
                        <button id="btn-annuler-reponse<?php echo $i; ?>" class="reponse-comm-btn-annuler"> Annuler
                        </button>
                    </article>
                    <?php
    ...
    let
    <!-- perlet de boucler sur toutes les notes -->
    <?php for ($i = 0; $i < $nbrNotes; $i++): ?>
        <script>
            /** Passe le bloc de réponse en (display : flex) lors d'un clic sur le bouton Répondre */
            function Repondre() {
                let label_reponse = document.getElementById(
                    "label-reponse<?php echo $i; ?>"); /** recherche de chaque label via leur id php */
                let texte_reponse = document.getElementById(
                    "texte-reponse<?php echo $i; ?>"); /** recherche de chaque texte via leur id php */
            ...
            function ValiderReponse() {
                ...
                document.cookie = "id_commentaire = " + btn_repondre<?php echo $i; ?>
                    .value; /** conservation de la valeur de texte rentrée et de l'id_commentaire via un cookie pour la réutiliser en php (cf haut de la page)*/

                window.location.reload();
            }
            let btn_repondre<?php echo $i; ?> = document.getElementById(
                "réponse<?php echo $i; ?>"); /** recherche de chaque bouton "Répondre" via leur id php */
            if (btn_repondre<?php echo $i ?> != null) {
                btn_repondre<?php echo $i; ?>.addEventListener('click', Repondre);

                let btn_annuler_reponse<?php echo $i; ?> = document.getElementById(
                    "btn-annuler-reponse<?php echo $i; ?>"); /** recherche de chaque bouton "Annuler" via leur id php */
                btn_annuler_reponse<?php echo $i; ?>.addEventListener('click', AnnulerRepondre);

                let btn_valider_reponse<?php echo $i; ?> = document.getElementById(
                    "btn-valider-reponse<?php echo $i; ?>"); /** recherche de chaque bouton "Envoyer" via leur id php */
    ...
    ?>
        ...
        <script>
            ...
            let etoile<?php echo $cpt2; ?> = document.getElementById(
                "etoile<?php echo $cpt2; ?>"); /** créer une variable pour chaque valeur d'étoile */
            etoile<?php echo $cpt2; ?>.addEventListener("mouseover",
                PassageEtoilesDorés<?php echo $cpt2; ?>); /** Event lors du passage de la souris sur une étoile */
            etoile<?php echo $cpt2; ?>.addEventListener("click",
                PassageEtoilesDorésPermanent<?php echo $cpt2; ?>); /** Event lors d'un click de la souris sur une étoile */
            let perm = 0;
        </script>
    ...
    <script>
        ...
        function ValiderCommentaire() {
            let nbetoiles = document.getElementsByClassName("nb").length;
            if (nbetoiles == 0) {
                nbetoiles = 1;
            }
        ...
        let bloc_comm = document.getElementsByClassName("bloc-ajout-commentaire")[0];

        let image_centrale = document.getElementsByClassName("photo-prod1")[0];

        if (document.getElementsByClassName("photo_min_1")[0] != null) {
            let image1 = document.getElementsByClassName("photo_min_1")[0];
            image1.addEventListener("click", PassageImageCentrale1);
        }

        if (document.getElementsByClassName("photo_min_2")[0] != null) {
            let image2 = document.getElementsByClassName("photo_min_2")[0];
            image2.addEventListener("click", PassageImageCentrale2);
        }

        if (document.getElementsByClassName("photo_min_3")[0] != null) {
            let image3 = document.getElementsByClassName("photo_min_3")[0];
            image3.addEventListener("click", PassageImageCentrale3);
        }


        let btn_ajout_avis = document.getElementsByClassName("ajouter-avis")[0];
        if (btn_ajout_avis != null) {
            btn_ajout_avis.addEventListener('click', AfficheComm);

            let bloc_etoile = document.getElementsByClassName("bloc-étoile")[0];
            bloc_etoile.addEventListener("mouseout", PassageEtoilesGrises);


            let texte_commentaire = document.getElementById("texte-commentaire");

            let btn_ajouter_commentaire = document.getElementById("btn-valider-commentaire");
            btn_ajouter_commentaire.addEventListener("click", ValiderCommentaire);

            let btn_annuler_commentaire = document.getElementById("btn-annuler-commentaire");
